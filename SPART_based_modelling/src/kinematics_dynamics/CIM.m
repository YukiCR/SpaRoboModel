...
n_q=robot.n_q; % number of active joints
n=robot.n_links_joints; % Number of manipulator links and joints (includes fixed joints, and excludes the base-link).
...
for i=n:-1:1
    ...
    child=find(robot.con.child(:,i))'; % find all child of i, only 1 for serial robot
...
Mdot0_tilde = Mdot0;
%Add children contributions
child=find(robot.con.child_base)'; % find all child of base
for j=1:length(child)
